...
- **`POST /triage` returns a 500 mentioning `OPENAI_API_KEY`:** the server process itself is missing the key (see Local Setup step 3) — this is a deliberate, clear error from a `RuntimeError` exception handler in `src/api/main.py`, not a bug.
