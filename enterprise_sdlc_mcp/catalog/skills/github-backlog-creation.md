...
1. Identify the functional requirement in `{{project.docs.product_brief}}` the work serves.
...
3. Create the technical task issue(s) as usual, then link each as a **GitHub sub-issue** of the story.
4. Non-user-facing enabling work that doesn't map to a single FR is grouped under an **epic** issue (`type:epic`) instead of a story.
5. Update the traceability table in the product brief when a new story is added.

## Checklist

- [ ] Issue has a clear, action-oriented title.
- [ ] Work type, priority, SDLC phase, and component/agent-role labels applied per the repository label taxonomy.
- [ ] Assigned to the correct milestone (e.g. `{{project.milestone.current}}`).
- [ ] Added to the project board with fields set (Work Type, Priority, SDLC Phase, Component/Agent, Slice).
